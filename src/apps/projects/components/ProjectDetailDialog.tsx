...
import { ExternalLink, Github, Code2, Share } from "lucide-react";
...
export function ProjectDetailDialog({
  ...
  // Get shareable URL - prioritize liveUrl, then githubUrl
  const getShareableUrl = (project: Project): string | null => {
            ...
            <div className="flex gap-2 pt-1">
              {project.liveUrl && (
                <Button
                  variant="retro"
                  size="sm"
                  className={`font-geneva-12 text-[11px] h-7 min-h-7 flex items-center gap-1 hover:[border-image:url('/button-default.svg')_60_stretch] ${
                    getShareableUrl(project) ? 'w-1/3' : 'w-1/2'
                  }`}
              ...
              {getShareableUrl(project) && (
                <Button
                  variant="retro"
                  size="sm"
                  className={`font-geneva-12 text-[11px] h-7 min-h-7 flex items-center gap-1 hover:[border-image:url('/button-default.svg')_60_stretch] ${
                    (project.liveUrl && project.githubUrl) ? 'w-1/3' : 
                    (project.liveUrl || project.githubUrl) ? 'w-1/2' : 'w-full'
                  }`}
                  onClick={handleShare}
                >
