...
async function checkAndIncrementAIMessageCount(
  ...
  if (isAuthenticated && authToken) {
    ...
    // If the request is from ryo or richard **and** the token is valid, bypass rate limits entirely
    if (isRyo || isRichard) {
      return { allowed: true, count, limit };
    }
  }

  // If the user *claims* to be ryo or richard but is **not** authenticated, deny the request outright
  if (isRyo || isRichard) {
    return {
      allowed: false,
      count: 0,
